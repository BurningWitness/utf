...
import           Codec.Encoding.UTF8 as UTF8 (Error (..))
...
deriving instance Eq r => Eq (Result r)


conformance :: (IsString a, IsString b) => (a -> b -> Expectation) -> Spec
conformance f = do
...
deriving instance Show UTF8.Error
deriving instance Eq UTF8.Error



utf8 :: Spec
utf8 =
  describe "UTF-8 conformance" $ do
    describe "Attoparsec" $ do
      describe "Hoehrmann" $
        conformance $ \a b ->
          parse (hoehrmannP . HoehrmannHandler $ Replace '\xFFFD') a `shouldBe` Done "" b

      describe "Naive" $
        conformance $ \a b ->
          parse (naiveP . NaiveHandler $ \_ -> Replace '\xFFFD') a `shouldBe` Done "" b

    describe "Text" $ do
      describe "Hoehrmann" $ do
        xdescribe "Strict" $
          conformance $ \a b ->
            Strict.decodeUtf8With (replace '\xFFFD') a `shouldBe` b

        xdescribe "Lazy" $
          conformance $ \a b ->
            Lazy.decodeUtf8With (replace '\xFFFD') a `shouldBe` b

      describe "Naive SIMD" $ do
        describe "Strict" $
          conformance $ \a b ->
            decodeSimdStrict (NaiveHandler $ \_ -> Replace '\xFFFD') a `shouldBe` Right b

        describe "Lazy" $
          conformance $ \a b ->
            decodeSimdLazy (NaiveHandler $ \_ -> Replace '\xFFFD') a `shouldBe` Right b

      describe "Naive" $ do
        describe "Strict" $
          conformance $ \a b ->
            decodeStrict (NaiveHandler $ \_ -> Replace '\xFFFD') a `shouldBe` Right b

        describe "Lazy" $
          conformance $ \a b ->
            decodeLazy (NaiveHandler $ \_ -> Replace '\xFFFD') a `shouldBe` Right b
